...
namespace EventBus.AzureServiceBus
{
    public class EventBusServiceBus : BaseEventBus
    {
        ...
        private ISubscriptionClient CreateSubscriptionClientIfNotExist(string eventName)
        {
            ...
            var exists = managementClient.SubscriptionExistsAsync(EventBusConfig.DefaultTopicName, GetName(eventName)).GetAwaiter().GetResult();

            if (!exists)
            {
                managementClient.CreateSubscriptionAsync(EventBusConfig.DefaultTopicName, GetName(eventName)).GetAwaiter().GetResult();
        ...
        private void CreateRuleIfNotExists(string eventName, ISubscriptionClient subscriptionClient)
        {
            ...
            try
            {
                var rule = managementClient.GetRuleAsync(EventBusConfig.DefaultTopicName, GetName(eventName), eventName).GetAwaiter().GetResult();
        ...
        {
            return new SubscriptionClient(EventBusConfig.EventBusConnectionString, EventBusConfig.DefaultTopicName, GetName(eventName));
        }

        public override void UnSubscribe<T, TH>()
        {
            var evetName = typeof(T).Name;
            try
            {
                var subscriptionClient = CreateSubscriptionClient(evetName);

                subscriptionClient.
                    RemoveRuleAsync(evetName).
                    GetAwaiter().
                    GetResult();
            }
            catch(MessagingEntityNotFoundException)
            {
                logger.LogWarning("The messaging entity {eventName} could not be found", evetName);
            }

            logger.LogWarning("Unsubcribing from event {EventName}", evetName);
